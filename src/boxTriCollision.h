...
void createEdges(
	std::vector<std::shared_ptr<BoxEdge> > &edges, // output
...
public:
	...
	//   edge1 = -1
	//   edge2 = -1
	//   cornerEdges = [d, e, f], where d, e, f, are the edge indicies for the box
	// Edge-Edge:
	//   count1 = 2
	//   count2 = 2
	...
	//   tri1 = i, where i is the triangle index for cloth
	//   tri2 = -1
	//   edge1 = -1
	//   edge2 = -1
	//   cornerEdges = [0, 0, 0]
	...
	Eigen::Vector3d tan1; // tangent on box
	Eigen::Vector3d bin1; // binormal on box
	...
	Eigen::Vector2i adjf; // Adjacent faces for an edge
	int edge1; // edge index for box
	int edge2; // edge index for cloth
	Eigen::Vector3i cornerEdges; // indices of edges that connect box corner vertices
	Eigen::Vector3d cornerEdgeWeights; // corresponding weights (0s and 1s)
...
void boxTriCollision(
	...
	const Eigen::VectorXi &isEOL2,
	bool justVerts2,
	const std::vector<std::shared_ptr<BoxEdge> > &edges2);
